...
class BinaryLogReg:
    ...
    def __init__(self, _lambda: float = 1e-3):
        ...
        self.weight: np.ndarray = np.zeros(784)
        self.bias: float = 0.0
    ...
    def mu(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        margins = y * (self.bias + X @ self.weight)
        mu = np.empty_like(margins, dtype=float)
        positive = margins >= 0
        mu[positive] = 1 / (1 + np.exp(-margins[positive]))
        exp_margins = np.exp(margins[~positive])
        mu[~positive] = exp_margins / (1 + exp_margins)
        return mu
    ...
    def loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        """
        margins = y * (self.bias + X @ self.weight)
        data_loss = np.mean(np.logaddexp(0, -margins))
    ...
    def gradient_J_weight(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        mu = self.mu(X, y)
        return (X.T @ ((mu - 1) * y)) / len(X) + 2 * self._lambda * self.weight
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        """
        scores = self.bias + X @ self.weight
    ...
    def train(
        ...
        for _ in range(epochs):
            ...
            result["train_errors"].append(
                self.misclassification_error(X_train, y_train)
            )
...
if __name__ == "__main__":
    ...
    experiments = [
        ("gd", len(x_train), 0.5, 80),
        ("sgd1", 1, 0.001, 8),
        ("sgd100", 100, 0.05, 30),
    ]

    for name, batch_size, learning_rate, epochs in experiments:
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig(output_dir / f"{name}_objective.png", bbox_inches="tight")
        plt.cla()
        plt.clf()
        ...
        plt.legend()
        plt.savefig(
            output_dir / f"{name}_misclassification.png", bbox_inches="tight"
        )
        plt.cla()
        plt.clf()
