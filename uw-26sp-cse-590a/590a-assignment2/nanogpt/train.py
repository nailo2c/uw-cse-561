...
def parse_args():
    ...
    parser.add_argument(
        "--micro-batch-size", type=int, default=None, help="Override micro_batch_size from config (useful for setup verification on memory-constrained runs)",
    )
    parser.add_argument(
        "--max-steps", type=int, default=None, help="Stop training after this many steps (overrides total_train_steps from config)",
    )
...
if __name__ == "__main__":
    ...
    if args.micro_batch_size is not None:
        cfg.hparams.micro_batch_size = args.micro_batch_size
    if args.max_steps is not None:
        if args.max_steps <= 0:
            raise ValueError(f"--max-steps must be positive, got {args.max_steps}")
        cfg.hparams.total_train_steps = args.max_steps
        cfg.hparams.warmup_steps = min(
            cfg.hparams.warmup_steps,
            max(0, args.max_steps - 1),
        )
