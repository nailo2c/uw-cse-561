...
def run_static_batching(model, cfg, requests, batch_size=4, verbose=True):
    ...
    for b in batches:
        # Select the next static batch.  The last group may be short; pad it by
        # repeating the final real request so the compiled batch shape stays fixed.
        real_batch = requests[b * batch_size : (b + 1) * batch_size]
        batch = list(real_batch)
        while len(batch) < batch_size:
            batch.append(batch[-1])

        batch_admit = time.perf_counter()
        # Static batching runs until the longest request in the group finishes.
        decode_steps = max(r["decode_len"] for r in batch)

        # Left-pad requests to a common prompt length and stack them.
        input_ids, segment_ids = _make_batch(batch, pad_id)

        with jax.set_mesh(cfg.mesh):
            t0 = time.perf_counter()

            last_token, cache = _prefill(
                model, input_ids, segment_ids,
                static_cache_template, head_dim, pad_id=pad_id,
            )
            ...
            t0 = time.perf_counter()
            generated, _ = _decode_loop(
                model, last_token, cache, head_dim,
        ...
        for req_idx, req in enumerate(real_batch):
        ...
        total_tok += sum(r["prompt_len"] + r["decode_len"] for r in real_batch)
...
def run_continuous_batching(model, cfg, requests, num_slots=4, num_decode_steps=32,
                            verbose=True):
    """
    Continuous batching: keep a pool of active slots, run fixed-size decode
    chunks, retire completed requests, and immediately admit pending requests.
    """
    head_dim = cfg.model.attn.head_dim
    pad_id = cfg.model.vocab_size - 1
    ...
    def admit_request(slot_idx, req):
        nonlocal shared_cache, jax_key, t_prefill, t_scatter, n_admits, total_tokens
    ...
    while any(slot_active):
        # Build a fixed-shape batch of last tokens.  Inactive slots receive a
        # harmless dummy token; their cache rows will be overwritten on admit.
        batch_last_tokens = jnp.concatenate(
            [
                slot_last_token[s] if slot_active[s] else inactive_tok
        ...
        t0 = time.perf_counter()
        with jax.set_mesh(cfg.mesh):
            generated, shared_cache = _decode_loop(
                model, batch_last_tokens, shared_cache, head_dim,
        ...
        for s in range(num_slots):
            ...
            remaining = max(slot_max_gen[s] - slot_gen_count[s], 0)
            useful_tokens = min(num_decode_steps, remaining)
            slot_token_log[s].extend(gen_np[:, s].tolist())
            slot_gen_count[s] += num_decode_steps
            slot_last_token[s] = generated[-1:, s:s + 1]
            total_tokens += useful_tokens

            request_finished = slot_gen_count[s] >= slot_max_gen[s]
            if request_finished:
                ...
                slot_token_log[s] = []
                slot_active[s] = False
                slot_last_token[s] = inactive_tok
                slot_gen_count[s] = 0
                slot_max_gen[s] = 0
                slot_admit_time[s] = 0.0
                slot_decode_start[s] = 0.0
                slot_request_id[s] = -1
                slot_prompt_len[s] = 0
                n_rounds += 1
